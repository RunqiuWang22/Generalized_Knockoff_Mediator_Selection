library(dplyr)
library(tidyr)
setwd("/Users/runqiuwang/Documents/mediation/simulation/Mediation_continuous_nonlinear/change_rho_0106_interaction/change_rho_0106_interaction/")
Allfinal=NULL
for (p in c(100)){
  for (a in seq(0.4,1.0,by=0.2)){
    for (b in c(0.5,1,1.5,2)){
      for (ss in c(0.7)){
        for (std in c(0.4)){
          for (setting in c("S2")){
            for (sigma in c("CS")){
