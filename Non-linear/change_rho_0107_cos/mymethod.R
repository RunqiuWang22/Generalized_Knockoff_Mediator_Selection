...
mymethod <- function(X,Y,M,V=NULL,q,scale=T,type){
  ...
  if (type=="non-linear"){
    ...
    #fit3 = randomForest(Y ~.,importance=TRUE,data=fdata,family="gaussian")
    tmp=fit3$importance
    #varImpPlot(fit3)
    Z2 = tmp[(1:p)+1,1] 
  ...
  if (type=="non-linear_XGB"){
    ...
    fdata_X=fdata[,-1]
    fdata_Y=fdata[,1]
    dtrain <- xgb.DMatrix(data = fdata_X, label = fdata_Y)
    params <- list(
    ...
    xgb_model <- xgboost(data = fdata_X, label = fdata_Y, params = params, nrounds = 100, verbose = 0)
    importance <- xgb.importance(model = xgb_model)
    
    indices <- match(importance$Feature, colnames(fdata_X))
    tmp = rep(0,ncol(fdata_X))
    tmp[indices] = importance$Gain
    Z2 = tmp[(1:p)+1] 
    Z2tilde = tmp[(1:p)+1+p]
  }
  ### combine
