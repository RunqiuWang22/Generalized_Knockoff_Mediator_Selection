...
library(tidyr)
library(dplyr)
setwd("~/Documents/mediation/simulation/Mediation_continuous/confounding/change_rho_confounding/result")

generate_and_save_plot <- function(a, b, p, ss, std, setting, sigma) {   
  # Set working directory   
  
  # Construct the filename based on input parameters   
  file_name <- sprintf("result_%.2f_%.2f_%d_%.2f_%.2f_%s_%s.csv", a, b, p, ss, std, setting, sigma)   
  ...
  plot <- ggplot(df_long, aes(x = rho, y = Value, color = Method, linetype = Method)) + 
    geom_line(size = 1) + 
    geom_point(size = 2) + 
    ...
    scale_x_continuous(limits = c(0, 0.9), breaks = seq(0, 0.9, by = 0.1)) + 
    scale_y_continuous(expand = expansion(mult = c(0, 0.05)), limits = c(0, NA)) + 
    scale_color_manual(values = custom_colors) + 
    scale_linetype_manual(values = custom_linetypes) +
    labs(
      title = bquote(a == .(a) ~"," ~ b == .(b) ~"," ~ p == .(p)),
      x = bquote("Correlation " ~ rho),
      y = NULL,
      color = NULL,
      linetype = NULL
    ) + 
  ...
  output_file <- sprintf("plot_conf_rho_%.2f_%.2f_%d_%.2f_%.2f_%s_%s.pdf", a, b, p, ss, std, setting, sigma)   
    ...
    for (b in c(0.3,0.5)){
      for (ss in c(0.7)){
        for (std in c(0.4)){
          for (setting in c("S2")){
            for (sigma in c("CS")){
              output_file <- generate_and_save_plot(a=a, b = b, p = p, ss = ss, std=std, setting=setting,sigma=sigma)
            }
          }
        }
      }
