...
generate_and_save_plot <- function(rho, b, p, ss, std, setting, sigma) {   
  # Set working directory   
  
  # Construct the filename based on input parameters   
  file_name <- sprintf("result_%.2f_%.2f_%d_%.2f_%.2f_%s_%s.csv",rho,b,p,ss,std,setting,sigma)
  ...
  plot <- ggplot(df_long, aes(x = a, y = Value, color = Method, linetype = Method)) + 
    geom_line(size = 1) + 
    geom_point(size = 2) + 
  ...
  output_file <- sprintf("plot_conf_a_%.2f_%.2f_%d_%.2f_%.2f_%s_%s.pdf", rho, b, p, ss, std, setting, sigma)   
    ...
    for (b in c(0.3,0.5)){
      for (ss in c(0.7)){
        for (std in c(0.4)){
          for (setting in c("S2")){
            for (sigma in c("CS")){
output_file <- generate_and_save_plot(rho=rho, b = b, p = p, ss = ss, std=std, setting=setting,sigma=sigma)
      }
    }
  }
}
