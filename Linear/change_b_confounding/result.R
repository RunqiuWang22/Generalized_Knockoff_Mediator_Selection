...
# Define the function to create and save the plot
generate_and_save_plot <- function(a, rho, p, ss,std,setting,sigma) {
  # Set working directory

  # Construct the filename based on input parameters
  file_name <- sprintf("result_%.2f_%.2f_%d_%.2f_%.2f_%s_%s.csv",a,rho,p,ss,std,setting,sigma)
  ...
  plot <- ggplot(df_long, aes(x = b, y = Value, color = Method, linetype = Method)) + 
    geom_line(size = 1) + 
    geom_point(size = 2) + 
    ...
    scale_x_continuous(limits = c(0.1, 0.5), breaks = seq(0.1, 0.5, by = 0.1)) + 
    scale_y_continuous(expand = expansion(mult = c(0, 0.05)), limits = c(0, NA)) + 
    scale_color_manual(values = custom_colors) + 
    scale_linetype_manual(values = custom_linetypes) +
    labs(
      title = bquote(a == .(a) ~"," ~ rho == .(rho) ~"," ~ p== .(p)),
      x = "b",
      y = NULL,
      color = NULL,
      linetype = NULL
  ...
  output_file <- sprintf("plot_conf_b_%.2f_%.2f_%d_%.2f_%.2f_%s_%s.pdf", a, rho, p, ss, std,setting,sigma)
    ...
    for (a in c(0.2)){
      for (ss in c(0.7)){
        for (std in c(0.4)){
          for (setting in c("S2")){
            for (sigma in c("CS")){
output_file <- generate_and_save_plot(a=a, rho = rho, p = p, ss = ss, std=std, setting=setting,sigma=sigma)
      }
    }
  }
}
